//comments und diese hinzufügen und beabeiten und so ...<br>
<h2>
	{$title}
</h2>
...
<div class="filmCommentSection">
	{if count($commentsUser) > 0 or count($commentsNonuser) > 0}
		{foreach $commentsUser as $comment}<div id="usercomment" data-cid="{$comment["cid"]}"><strong>Du hast geschrieben:</strong><button id="btn-editUsercomment">bearbeiten</button><br><div>{$comment["text"]}</div><br></div>{/foreach}
